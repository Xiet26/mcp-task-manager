...
// Ensure project root directory path
...
export async function updateTask(
  ...
  // Check if task is completed, completed tasks are not allowed to be updated (unless explicitly allowed)
  if (tasks[taskIndex].status === TaskStatus.COMPLETED) {
    // Only allowed to update summary field (task summary) and relatedFiles field
...
export async function batchCreateOrUpdateTasks(
  ...
  // This will be used later to parse dependencies
...
export async function canExecuteTask(
  ...
  if (task.status === TaskStatus.COMPLETED) {
    return { canExecute: false }; // Completed tasks do not need to be executed
  }
...
export async function assessTaskComplexity(
  ...
  if (level === TaskComplexityLevel.LOW) {
    recommendations.push("This task has low complexity, can be executed directly");
  ...
  // Medium complexity task suggestions
    ...
    recommendations.push("Can be executed in stages and periodically check progress, ensuring understanding accurate and complete implementation");
  ...
  // High complexity task suggestions
    ...
    recommendations.push("This task has high complexity, suggest performing thorough analysis and planning");
    recommendations.push("Consider breaking down task into smaller, independently executable sub-tasks");
    recommendations.push("Establish clear milestones and checkpoints, facilitating tracking progress and quality");
    if (
      dependenciesCount > TaskComplexityThresholds.DEPENDENCIES_COUNT.MEDIUM
    ) {
      recommendations.push(
        "Many dependent tasks, suggest making dependency relationship diagram, ensuring correct execution sequence"
      );
    }
  }
  // Very high complexity task suggestions
  else if (level === TaskComplexityLevel.VERY_HIGH) {
    recommendations.push("⚠️ This task has very high complexity, strongly suggest splitting into multiple independent tasks");
    recommendations.push(
      "Perform thorough analysis and planning before execution, clearly define scope and interface of each sub-task"
    );
    recommendations.push(
      "Perform risk assessment, identify possible obstacles and develop countermeasures"
    );
    recommendations.push("Establish specific test and verification standards, ensuring output quality of each sub-task");
    if (
      descriptionLength >= TaskComplexityThresholds.DESCRIPTION_LENGTH.VERY_HIGH
    ) {
      recommendations.push(
    ...
    if (dependenciesCount >= TaskComplexityThresholds.DEPENDENCIES_COUNT.HIGH) {
      recommendations.push(
        "Too many dependent tasks, suggest re-evaluating task boundaries, ensuring task splitting reasonable"
      );
    }
  }
...
export async function clearAllTasks(): Promise<{
  ...
  try {
    ...
    // Write to memory directory (only include completed tasks)
    await fs.writeFile(
      memoryFilePath,
...
export async function searchTasksWithCommand(
  ...
  try {
    ...
    // If there is search command, execute it
    if (cmd) {
      try {
  ...
  // Filter current task list based on conditions
  ...
  // Sort - Sort by update or completion time descending
  allTasks.sort((a, b) => {
    // Priority sort by completion time
    if (a.completedAt && b.completedAt) {
      return b.completedAt.getTime() - a.completedAt.getTime();
    } else if (a.completedAt) {
      return -1; // a completed but b not, a ahead
    } else if (b.completedAt) {
      return 1; // b completed but a not, b ahead
    }
  ...
  const safePage = Math.max(1, Math.min(page, totalPages || 1)); // Ensure page number valid
...
function generateSearchCommand(
  ...
  if (isWindows) {
    ...
    if (isId) {
      ...
      // Multi-keyword search - Windows using PowerShell
    ...
    if (isId) {
      ...
      // Multi-keyword use pipe to connect multiple grep commands
...
/**
 * Safely escape shell parameter, prevent command injection
 */
