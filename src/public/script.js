...
); // Assume this element exists in HTML
const langSwitcher = document.getElementById("lang-switcher"); // << New: Get language switcher element
...
function detectLanguage() {
  ...
  if (browserLang) {
    ...
    if (browserLang.toLowerCase().startsWith("zh")) return "zh-TW"; // Fallback simplified to traditional
...
function applyTranslations() {
  ...
  document.querySelectorAll("[data-i18n-key]").forEach((el) => {
    ...
    if (el.hasAttribute("placeholder")) {
      ...
      // If needed, can also translate value, but usually not necessary
    } else {
  ...
  // For example, if footer time format needs localization, handle here
  // updateCurrentTime(); // Ensure time format may also update (if needed)
}
...
async function initI18n() {
  ...
  // << New: Set initial value for switcher >>
  if (langSwitcher) {
    ...
    .then(() => {
      ...
      if (selectedTaskId) {
        ...
        if (task) {
          ...
          // If the selected task no longer exists, clear details
      ...
      // Ensure dropdown menu value is consistent with current language
...
async function fetchTasks() {
  try {
    ...
    // Extract global analysis result (find the first non-empty)
    ...
    // Only update if the found result is different from the current stored
    ...
    // Simple comparison of task count or identifier to decide whether to re-render
    // Ideal situation should compare each task's content and perform DOM update
    ...
    if (tasksChanged) {
      ...
      // If no need to re-render list, may only need to update progress bar
      updateProgressIndicator();
      // Consider whether to update graph (if status may change)
...
function setupSSE() {
  ...
  evtSource.onmessage = function (event) {
    ...
    // Can perform more complex judgment based on event.data content, currently just update when receiving message
  };
  ...
  evtSource.onerror = function (err) {
    ...
    // Delay a period of time and try to reconnect
...
// New: Helper function to compare task lists for changes (most comprehensive version)
...
// New: Show temporary error message function
...
function selectTask(taskId) {
  ...
  // If clicked the same task again, then unselect
  if (selectedTaskId === taskId) {
    selectedTaskId = null;
    taskDetailsContent.innerHTML = `<p class="placeholder">${translate(
      "task_details_placeholder"
    )}</p>`;
    highlightNode(null); // Unhighlight
    return;
  }
  ...
  // 1. Create basic skeleton (use innerHTML, but replace dynamic content with ID-less empty elements)
  taskDetailsContent.innerHTML = `
    ...
    <div class="task-details-section">
      <h4>Dependencies (Preceding Tasks)</h4>
  ...
  // New: Fill Summary (if exists and completed)
...
function renderDependencyGraph() {
  if (!dependencyGraphElement || !window.d3) {
    ...
    if (dependencyGraphElement) {
      // First or D3 loss time display prompt, do not clear existing graph
  ...
  // If no task, clear graph and display prompt
  if (tasks.length === 0) {
    dependencyGraphElement.innerHTML = `<p class="placeholder">${translate(
      "dependency_graph_placeholder_empty"
    )}</p>`;
    // Reset SVG and simulation variables so next time correct initialization
    svg = null;
    g = null;
    simulation = null;
    return;
  ...
  if (!svg) {
    ...
    g = svg.append("g"); // Main group, used for zooming and panning
  ...
  const nodeEnter = nodeSelection
    ...
    .attr("data-id", (d) => d.id)
    // Initial position: Appear from simulated calculation position (if exists) or random position, initial scale 0
    .attr(
      "transform",
      (d) =>
  ...
  function drag(simulation) {
    ...
    function dragended(event, d) {
      ...
      // Cancel fixed position, let node continue to be influenced by force directed (if needed)
      // d.fx = null;
      // d.fy = null;
...
function ticked() {
  ...
  g.select(".nodes")
    .selectAll("g.node-item")
    // << Modified: Add coordinate backup value >>
...
function getNodeColor(nodeData) {
  switch (nodeData.status) {
    ...
    case "Pending":
      return "#f1c40f"; // Same as progress bar and status label
    default:
...
function updateCurrentTime() {
  const now = new Date();
  // Keep original format, if time localization needed, can handle here
...
function renderGlobalAnalysisResult() {
  ...
  // If element does not exist, try to create and add to appropriate location (e.g. header or main content before)
  if (!targetElement) {
...
// New: Highlight dependency graph node
...
// New: Helper function to get status class (should be placed after ticked function, before getNodeColor, or after)
